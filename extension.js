(function (Scratch) {
  ...
  class HyperLinkV1 {
    ...
    getInfo() {
      return {
        id: 'hyperlinkV1', // IDを更新
        name: 'HyperLink v1', // 表示名を「HyperLink v1」に固定
        color1: '#0080ff',
        blocks: [
          ...
          "---",
          { opcode: 'connectAndRegister', blockType: Scratch.BlockType.COMMAND, text: 'ラベル [LABEL] で URL [URL] を登録して接続', arguments: { LABEL: { type: Scratch.ArgumentType.STRING, defaultValue: 'API_1' }, URL: { type: Scratch.ArgumentType.STRING, defaultValue: 'https://api.example.com' } } },
          { opcode: 'setModeAndRun', blockType: Scratch.BlockType.COMMAND, text: 'ラベル [LABEL] を [MODE] モードへ変換', arguments: { LABEL: { type: Scratch.ArgumentType.STRING, defaultValue: 'API_1' }, MODE: { type: Scratch.ArgumentType.STRING, menu: 'modeMenu', defaultValue: '自動変換' } } },
    ...
    connectAndRegister(args) { this.apiRegistry[args.LABEL] = args.URL; this.manualDisconnect[args.LABEL] = false; if (!this.errorModes[args.LABEL]) this.errorModes[args.LABEL] = '1. 自動再接続'; this._startConnection(args.LABEL, args.URL, '自動変換'); }
    setModeAndRun(args) { const url = this.apiRegistry[args.LABEL]; if (url) this._startConnection(args.LABEL, url, args.MODE); }
    ...
    _startConnection(label, url, mode) {
      ...
      if (isWs) this._setupWS(label, url); else { this.isLooping[label] = true; this._httpLoop(label, url, mode === '直出力'); }
    }
    ...
    async _httpLoop(label, url, once) {
      do {
        try {
          ...
          if (this.lastData[label] !== text) { this.dataStorage[label] = text; this.lastData[label] = text; this._emit('onMessageReceived', label); }
        } catch (e) {
          this._emit('onError', label);
          if (this.connectedState[label]) { this.connectedState[label] = false; this._emit('onDisconnected', label); }
          if (this.errorModes[label] === '2. ハットでの対応') { this.isLooping[label] = false; break; }
          if (!once) await new Promise(r => setTimeout(r, 3000));
        }
        if (once) break; await new Promise(r => setTimeout(r, 2000));
    ...
    _setupWS(label, url) {
      ...
      ws.onmessage = (e) => { this.dataStorage[label] = e.data; this.lastData[label] = e.data; this._emit('onMessageReceived', label); };
      ws.onclose = () => {
        if (this.connectedState[label]) { this.connectedState[label] = false; this._emit('onDisconnected', label); }
        if (!this.manualDisconnect[label] && this.errorModes[label] === '1. 自動再接続') { setTimeout(() => { if (this.apiRegistry[label]) this._setupWS(label, url); }, 3000); }
      };
      ws.onerror = () => ws.close();
    }
    ...
    disconnectLabel(args) {
      ...
      this.manualDisconnect[label] = true; this.isLooping[label] = false;
      if (this.sockets[label]) { this.sockets[label].onclose = null; this.sockets[label].close(); delete this.sockets[label]; }
